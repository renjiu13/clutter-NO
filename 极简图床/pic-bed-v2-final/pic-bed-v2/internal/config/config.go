...
type Config struct {
	...
	EnableAutoClean  bool     `json:"enable_auto_clean"`  // 🧹 自动清理：定期删除超过N天的文件
	KeepOriginalName bool     `json:"keep_original_name"` // 📄 原始文件名：保留上传文件名+随机后缀

	// 功能详细配置
	AllowedTypes   []string `json:"allowed_types"`   // 允许的文件类型白名单
	AutoCleanDays  int      `json:"auto_clean_days"` // 自动清理：超过多少天的文件被删除
